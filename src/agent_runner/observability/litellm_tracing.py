...
class TracedModel(Model):
    ...
    async def get_response(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with self._tracer.span("llm.call", self._get_span_attributes(), kind=SpanKind.CLIENT) as span:
            ...
            self._record_response(
                span,
                response,
                self._get_argument(traced_args, traced_kwargs, "model_settings", 2),
            )
            return response
    ...
    async def _stream_response(self, args: tuple[Any, ...], kwargs: dict[str, Any]) -> AsyncIterator[Any]:
        with self._tracer.span("llm.call", self._get_span_attributes(), kind=SpanKind.CLIENT) as span:
            traced_args, traced_kwargs = self._inject_model_headers(args, kwargs)
            self._record_request(span, traced_args, traced_kwargs)
            model_settings = self._get_argument(traced_args, traced_kwargs, "model_settings", 2)
    ...
    @staticmethod
    def _inject_model_headers(args: tuple[Any, ...], kwargs: dict[str, Any]) -> tuple[tuple[Any, ...], dict[str, Any]]:
