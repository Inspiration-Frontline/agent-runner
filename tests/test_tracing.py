...
def test_active_span_uses_lowercase_w3c_trace_id() -> None:
    with tracing.get_tracer().span("test.root") as span:
...
def test_extracts_w3c_parent_context() -> None:
    ...
    with tracing.get_tracer().span("test.child", parent_context=parent_context) as span:
...
def test_conversation_rpc_metadata_contains_active_w3c_context() -> None:
    with tracing.get_tracer().span("test.rpc") as span:
        metadata = dict(ConversationManagerClient._trace_metadata())
...
def test_exporter_setup_failure_does_not_break_span_creation(monkeypatch) -> None:
    ...
    monkeypatch.setattr(tracing, "_exporter_configured", False)

    tracer = tracing.init_tracer(endpoint="http://127.0.0.1:1", enabled=True)

    with tracer.span("test.exporter-unavailable") as span:
        assert TRACE_ID_PATTERN.fullmatch(span.trace_id)
